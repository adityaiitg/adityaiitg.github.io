...
* ML Objective: build dataset and model to predict the relevance score between user and a video
...
In the first stage, we use a light model to retrieve thousands of items from millions
...
- Social interactions: between players: to identify patterns of behavior, such as the formation of alliances, the sharing of resources, and the types of communication used between players.
...
- Keep the two-stage design explicit:
  - Retrieval: two-tower or graph-based candidate generation for high recall.
  - Ranking: richer cross features, sequence features, and multi-task heads.
- Train on watch-time-weighted labels and satisfaction proxies, not only clicks.
- Use feature freshness windows (minutes/hours/days) to capture short-term and long-term interests.
- Apply re-ranking constraints for creator diversity, repeated-item suppression, and policy safety.
- For cold start, blend content embeddings and popularity priors until interaction data accumulates.
