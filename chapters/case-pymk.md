...
* ML Formulation:
    ...
    * ML Category: two options: 
      ...
      * Graph representation (edge prediction)
        ...
        * input: social graph, predict edge between nodes 
...
* Online 
  * Number of friend requests sent over X time 
  * Number of friend requests accepted over X time 
...
* Feature selection
  ...
  * User: 
    ...
    * Interaction history (Number of likes, shares, comments)
...
* Model selection 
  * We choose GNN 
    ...
    * use similarities between node embeddings for edge prediction 
...
- Keep the PYMK stack as candidate generation (graph heuristics) + graph-aware scoring + policy re-rank.
- Candidate generation should combine multiple sources: friends-of-friends, shared org/school, co-engagement, contact import.
- Use graph embeddings and temporal features together (recent interactions often matter more than old links).
- Add suppression rules (already seen, blocked users, low-trust accounts, repeated declines).
- Include acceptance-quality metrics (accepted requests, later interaction quality), not request-send rate only.

### References
- PinSage (web-scale graph recommendation): https://arxiv.org/abs/1806.01973
