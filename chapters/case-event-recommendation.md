...
- Event recommendation is strongly constrained by time and geography; enforce strict candidate filters before ML ranking.
- Use availability-aware scoring: if inventory is low or event is near sell-out, include utility and fairness constraints.
- Handle sparse labels with transfer features (host quality, venue quality, category priors, social proof).
- Add lifecycle features (time-to-event, registration velocity, decay since announcement).
- Re-rank for user experience: diversity across event type, date window, and distance.

## References
- Two-stage recommendation pattern (retrieval + ranking): https://research.google/pubs/deep-neural-networks-for-youtube-recommendations/
- MMoE for multi-objective optimization: https://www.kdd.org/kdd2018/accepted-papers/view/modeling-task-relationships-in-multi-task-learning-with-multi-gate-mixture-
