...
* Users: 
  * Only use post author's info
    ...
    * violation history (Number of violations, Number of user reports, profane words rate)
* Context: 
  * Time of day, device
...
* HP tuniing: 
  * Number of hidden layers, neurons in layers, activation functions, learning rate, etc
...
* loss function: 
  ...
  * each task is a binary classification so e.g. CE for each task  
...
- Use policy-aligned taxonomy with separate decision thresholds by severity (warn, demote, remove, block).
- Keep human-in-the-loop review for ambiguous and high-impact cases; model output should include reason codes.
- Train with multimodal hard negatives (benign confounders) so models do not overfit single-modality shortcuts.
- Track calibration and drift by language, region, and content type; refresh thresholds regularly.
- Prefer precision at highest-severity action levels; use lower-threshold queues for human review.

### References
- Hateful Memes Challenge (multimodal confounders): https://arxiv.org/abs/2005.04790
