...
* relevance score 
  * weighted linear combination of: 
    ...
    * item popularity (e.g. number of reviews, or bought) (e.g. w=0.125), 
...
* Feature selection 
  ...
  * Item (product) features 
    ...
    * Reviews data (avg reviews, number of reviews, review textual data (text + embeddings)) 
    * category 
    * page rank 
    * engagement radius 
...
* Training Dataset
  ...
  * Pairwise approach: 
    ...
    * predict doc scores in a way that miimizes Number of inversions in the final ranked result 
...
- Use hybrid retrieval: lexical (inverted index/BM25) + semantic retrieval (dense embeddings) then fuse.
- Prefer multi-stage ranking for cost control:
  - Stage 1: cheap model for 10k->1k.
  - Stage 2: heavier LTR model for 1k->100.
  - Final re-ranker: business/policy constraints.
- Treat query understanding as first-class: spelling correction, normalization, intent/domain classification, and query reformulation.
- Add explicit debiasing and fairness checks (brand dominance, popularity bias, out-of-stock suppression).
- Monitor long-term health metrics: reformulation rate, zero-result rate, abandonment rate, and conversion quality.
...
- ColBERT (late interaction neural retrieval): https://arxiv.org/abs/2004.12832
- Faiss library (ANN at scale): https://arxiv.org/abs/2401.08281
