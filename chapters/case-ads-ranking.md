...
* Model selection 
  ...
  * NN
    ...
    * Cons for ads prediction: 
      ...
      * hard to capture pairwise interactions (large number of them)
...
- Use a two-stage serving path: fast retrieval (high recall) followed by a calibrated ranker (high precision).
- Optimize for expected value, not raw CTR only. Typical objective is expected utility = `p(click) * value_per_click` (or conversion value when available).
- Model delayed feedback explicitly. Conversions and post-click value arrive late; keep separate short-window and long-window labels.
- Correct position and logging bias with randomized exploration buckets and counterfactual estimators (IPS/SNIPS) before shipping model changes.
- Calibrate probabilities (for example isotonic/Platt) before auction or pacing decisions.
- Add hard guardrails in re-ranking: frequency capping, advertiser budget pacing, policy filters, and fatigue constraints.

### References
- Wide & Deep Learning for Recommender Systems: https://arxiv.org/abs/1606.07792
- Deep & Cross Network for Ad Click Predictions: https://arxiv.org/abs/1708.05123
- DCN V2 (web-scale LTR lessons): https://arxiv.org/abs/2008.13535
- Deep Interest Network for CTR Prediction: https://arxiv.org/abs/1706.06978
