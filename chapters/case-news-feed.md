...
* ML Formulation:
    ...
    * I/O: I: user_id, O: ranked list of unseen posts sorted by engagement score (weighted sum) 
...
* Feature selection 
  * Posts: 
    * Text
    * Image/videos
    * Number of reactions (likes, shares, replies, etc)
    * Age 
    * Hashtags 
  * User: 
    * ID, username
...
### Production-Grade Solution Notes
- Use a funnel architecture: candidate generation -> first-pass ranker -> heavy ranker -> policy re-ranker.
- Treat feed ranking as multi-objective optimization (click, like, comment, share, watch time, hide/report) with explicit trade-off weights.
- Prefer multi-task learning with shared trunk + task-specific heads to avoid training/serving many isolated models.
- Add freshness and creator-diversity controls in the final stage to reduce over-concentration.
- Include integrity penalties (spam probability, policy-risk scores) directly in the final ranking formula.
- Run continuous online experiments with guardrails (session quality, hide/report rate, crash/latency).

### References
- MMoE for multi-task recommendation: https://www.kdd.org/kdd2018/accepted-papers/view/modeling-task-relationships-in-multi-task-learning-with-multi-gate-mixture-
- YouTube two-stage recommendation architecture: https://research.google/pubs/deep-neural-networks-for-youtube-recommendations/
- LambdaMART ranking overview: https://www.microsoft.com/en-us/research/publication/from-ranknet-to-lambdarank-to-lambdamart-an-overview/
